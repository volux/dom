 ...
 *
 * @author modified by Andrey Skulov <andrey.skulov@gmail.com>
 */
...
class Css2Xpath
{
    ...
    protected static function _tokenize($expression)
    {
        $patterns = array(
             ...
             * @todo more elegance! :)
             */
            '/(^([a-z][a-z0-9_-]*(?!:)+))/i' => "*[local-name()=\"\${1}\"]",
            '/\[([a-z0-9_-]+)\]/i'                          => "*[@\${1}]",
        );
        $expression = preg_replace(array_keys($patterns), array_values($patterns), $expression);
